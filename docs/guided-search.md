...
7. Writes `app/recordings.json` — committed, ~131 entries, no transcript text:
...
5. Returns `[{id, title, collection, duration_s, summary, why, component_slugs}]` in the
   selector's order.

Tool result text for the model: one line per choice — `title (N min, collection): why`,
...
`POST /api/morning/sessions/{id}/listens` body `{recording_id}` → inserts a row and also
records `played` on the session's latest options message (`data.played = recording_id`) so
the card can show it and the prompt can say it. Response `{"ok": true}`.
...
serve better than silence, or when he asks for one. At most once per turn.
...
`[Guided meditation options: <inquiry> → <title>; <title> — played: <title>]`.
...
- Otherwise one row per choice: **title** · `N min` · collection, the *why* clause beneath in
...
- Layout, one row: `▶`/`❚❚` button · title (ellipsized) · `−15` · `+15` · a scrubber that
...
- `navigator.mediaSession` metadata carries the title and wires play/pause/seek so the lock
...
- `tests/test_recordings.py`: transcript parsing on a fixture (headers, folder-less Jhourney
  ...
  program snapshots, selector output parsing (unknown ids dropped, malformed → empty).
