...
def tool_result_text(choices: list[dict]) -> str:
    ...
    for choice in choices:
        line = (f"{choice['title']} ({_minutes(choice['duration_s'])} min, "
...
def replay_suffix(data: dict) -> str:
    ...
    titles = "; ".join(c["title"] for c in data["choices"])
    suffix = f" → {titles}" if titles else " → nothing fit"
    played = data.get("played")
    if played:
        title = next((c["title"] for c in data["choices"] if c["id"] == played), played)
        suffix += f" — played: {title}"
    return suffix
