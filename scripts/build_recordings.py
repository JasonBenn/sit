...
def load_component_map():
    """(normalized folder, lowercased mp3 name) -> [slug]; plus a basename fallback."""
    by_pair, by_base = {}, {}
...
def _one_summary(client, title, body):
    ...
    return " ".join(msg.content[0].text.split())
...
def main():
    ...
    by_pair, by_base = load_component_map()
    ...
    if not args.no_rsync:
        ...
        subprocess.run(
            ["rsync", "-a", "--partial", f"{args.out}/", NOSE_MEDIA],
            check=True,
        )
